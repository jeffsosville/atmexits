...
export default function SellerPortal() {
  ...
  async function loadData(authUserId: string) {
    // Get user profile
    ...
    if (!user) { router.replace('/seller-login'); return }
    setProfile(user)

    // Get their pending listings
    ...
    // Get deal rooms where they are the seller, with messages
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center">
      <div className="text-white">Loading your portal...</div>
    </div>
  )
            ...
            <div className="space-y-4">
              {listings.length === 0 ? (
                <div className="bg-gray-900 rounded-xl p-8 text-center border border-gray-800">
                  ...
                  <a href="/sell" className="inline-block bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg text-sm transition-colors">
                    List Your ATM Route →
                  </a>
                </div>
                ...
                <div key={listing.id} className="bg-gray-900 rounded-xl p-6 border border-gray-800">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <p className="font-semibold text-lg">{listing.machine_count} ATM Route</p>
                      <p className="text-gray-400 text-sm">
                        Submitted {new Date(listing.submitted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${statusColor(listing.status)}`}>
                      {listing.status}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      <p className="text-gray-500 text-xs mb-1">Machines</p>
                      <p className="font-medium">{listing.machine_count}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 text-xs mb-1">Monthly Surcharge</p>
                      <p className="font-medium">${listing.gross_monthly_surcharge?.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-gray-500 text-xs mb-1">Asking Price</p>
                      <p className="font-medium text-green-400">${listing.asking_price?.toLocaleString()}</p>
                    </div>
                  </div>
                  {listing.status === 'pending' && (
                    <p className="mt-4 text-xs text-yellow-500 bg-yellow-950 rounded-lg px-3 py-2">
                      ⏳ Under review by ATM Exits. You'll be notified once approved.
                    </p>
                  )}
                </div>
              ))}
                      ...
                      <div key={msg.id} className={`flex ${msg.sender_role === 'seller' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-sm px-4 py-3 rounded-2xl text-sm ${
                          ...
                          <p className="text-xs opacity-60 mb-1">
                            {msg.sender_name} · {new Date(msg.sent_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                          </p>
                          <p>{msg.body}</p>
                        </div>
                      </div>
